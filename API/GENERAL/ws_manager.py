...
class SocketManager:
    ...
    def __init__(self, all_online_users):
        self.__all_online_users = all_online_users
        self.__rooms = dict()  #for storing all rooms and its an nested dict

    async def emit(self, event: str, data: dict, ws: WebSocket,
                   room: int | str):
        ...
        if (room in self.__rooms) and (ws in self.__rooms[room].values()):
            for uid, wss in self.__rooms[room].items():
                if str(uid) == str(data["SENDER_ID"]):
                    continue
                else:
                    await wss.send_json({"event": event, "data": data})
        else:
            print(" im on else condition ")

    def join_room(self, room_id: str, user_id: str, ws: str):
        """ adds user to an group for listenong messages """
        self.__rooms.setdefault(str(room_id), {})[str(user_id)] = ws
        print("all connected clients arre ==>> ", self.__rooms)
        return True

    def leave_room(self, user_id: str, room_id: str):
        """ MAINLY DESIGNED FOR WHEN USER LEAVES AN GROUP CHAT """
        if room_id in self.__rooms and user_id in self.__rooms[room_id]:
            del self.__rooms[room_id][user_id]
        return True
    ...
    async def broadCastMessage(self, message, type: CodeType | str):
        """ Designed for any important notice by application admin to broadcast in all connected users """
        for ws in self.__all_online_users:
            await ws.send({
